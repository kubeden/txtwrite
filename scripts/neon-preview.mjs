...
if (!["create", "delete"].includes(command)) {
  throw new Error("Usage: node scripts/neon-preview.mjs <create|delete>");
}

function configuredBranchName() {
  if (args.branchName || process.env.NEON_BRANCH_NAME) {
    return args.branchName || process.env.NEON_BRANCH_NAME;
  }
...
if (!neonConfigured()) {
  if (process.env.NEON_REQUIRED === "true") {
    throw new Error("Neon is required but NEON_API_KEY or NEON_PROJECT_ID is missing.");
  }
...
if (command === "create") {
  ...
  const branch = existing
    ? {
        ...
        databaseUrl: await getConnectionUri({
          branchId: existing.id,
          databaseName,
          roleName,
          pooled
  ...
  if (!branch.databaseUrlRedacted) {
    branch.databaseUrlRedacted = redactDatabaseUrl(branch.databaseUrl);
  }
  ...
  const publicUrls = await resolvePublicUrls({
    ...
    dataApiAuthProvider:
      process.env.NEON_DATA_API_AUTH_PROVIDER || config.neon.publicUrls.dataApiAuthProvider,
