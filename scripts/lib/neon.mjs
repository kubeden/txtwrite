...
export async function deleteNeonBranch(branchId) {
  const projectId = process.env.NEON_PROJECT_ID;
  return neonRequest(`/projects/${projectId}/branches/${branchId}`, {
    method: "DELETE"
  });
}
...
async function createNeonAuth({ branchId, databaseName, authProvider }) {
  if (!authProvider) {
    throw new Error("NEON_AUTH_PROVIDER or neon.publicUrls.authProvider is required to provision Neon Auth.");
  }
  const projectId = process.env.NEON_PROJECT_ID;
  return neonRequest(`/projects/${projectId}/branches/${branchId}/auth`, {
    method: "POST",
...
async function addNeonAuthTrustedDomain({ branchId, domain, authProvider }) {
  ...
  if (!authProvider) {
    throw new Error("NEON_AUTH_PROVIDER or neon.publicUrls.authProvider is required to add Neon Auth domains.");
  }
...
export async function resolvePublicUrls({
  ...
  if (getAuthUrl || provisionAuth) {
    ...
    if (result.authUrl && authTrustedDomains.length > 0) {
      for (const domain of authTrustedDomains) {
        await addNeonAuthTrustedDomain({ branchId, domain, authProvider });
      }
    }
  }
  ...
  if (requirePublicUrls) {
    if ((getAuthUrl || provisionAuth) && !result.authUrl) {
      throw new Error("Neon Auth URL was required but could not be resolved.");
    }
    if ((getDataApiUrl || provisionDataApi) && !result.dataApiUrl) {
      throw new Error("Neon Data API URL was required but could not be resolved.");
    }
  }
