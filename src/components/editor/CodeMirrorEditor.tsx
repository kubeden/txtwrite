...
export default function CodeMirrorEditor({
  ...
  useEffect(() => {
    if (
      ...
      markdownText !== localViewRef.current.state.doc.toString()
    ) {
      ...
      try {
        ...
        console.error(
          "Error restoring cursor position after document change:",
          error,
        );
      }
  ...
  const handleLink = useCallback(() => formatText("[Link text](", ")"), [
    formatText,
  ]);
  ...
  const handleQuote = useCallback(() => formatMultiLine("> "), [
    formatMultiLine,
  ]);
  const handleUnorderedList = useCallback(() => formatMultiLine("- "), [
    formatMultiLine,
  ]);
  const handleCheckbox = useCallback(() => formatMultiLine("- [ ] "), [
    formatMultiLine,
  ]);
  ...
  const handleOrderedList = useCallback(() => formatOrderedList(), [
    formatOrderedList,
  ]);
