...
export default function DocumentTabs({
    ...
    const checkMobileScrollPosition = () => {
        ...
        // Only show right arrow if total content width exceeds visible width
        const hasMoreToScroll = Math.floor(scrollWidth - (scrollLeft + clientWidth)) > 2;
    ...
    useEffect(() => {
        if (mobileTabsRef.current && activeDocumentId) {
            const activeTab = mobileTabsRef.current.querySelector(`[data-doc-id="${activeDocumentId}"]`);
            if (activeTab) {
                ...
                const scrollTo = tabsContainer.scrollLeft + (tabRect.left - containerRect.left) -
            ...
            <div className="md:hidden relative px-2 py-2">
                ...
                {mobileLeftArrow && (
                    <div className="absolute left-0 top-0 bottom-0 w-8 z-10">
                        <div className="absolute inset-0 bg-gradient-to-r from-neutral-100 dark:from-neutral-900 to-transparent pointer-events-none"></div>
                        <button
                ...
                >
                    {documentTabs.map(tab => (
                        <div
                            key={tab.id}
                            data-doc-id={tab.id}
                            className={`px-3 py-2 ${activeDocumentId === tab.id
                ...
                {mobileRightArrow && (
                    <div className="absolute right-0 top-0 bottom-0 w-8 z-10">
                        <div className="absolute inset-0 bg-gradient-to-l from-neutral-100 dark:from-neutral-900 to-transparent pointer-events-none"></div>
                        <button
            ...
            <div className="hidden md:flex items-center relative px-3 text-sm">
                ...
                {showLeftArrow && (
                    <div className="absolute left-3 top-0 bottom-0 w-50 z-10 pointer-events-none">
                        <div className="rounded-md absolute inset-0 bg-gradient-to-r from-brand-light dark:from-brand-dark to-transparent"></div>
                        <button
                ...
                >
                    ...
                    {documentTabs.map(tab => (
                        <div
                            key={tab.id}
                            className={`px-3 py-1.5 ${activeDocumentId === tab.id ? ' text-neutral-600 dark:text-neutral-400' : 'text-neutral-400 dark:text-neutral-600'} my-1 rounded-md cursor-pointer group flex items-center shrink-0 w-30`}
                            onClick={() => handleDocTabChange(tab.id)}
                        >
                            {editingTitleId === tab.id ? (
                                ...
                                <div className="flex items-center justify-between w-full">
                                    <span
                                        ...
                                        onDoubleClick={(e) => startEditingTitle(e, tab.id, tab.title)}
                                        title={tab.title}
                                    >
                                        {tab.title}
                                    </span>
                ...
                {showRightArrow && (
                    <div className="absolute right-4 top-0 bottom-0 w-50 z-10 pointer-events-none">
                        <div className="rounded-md absolute inset-0 bg-gradient-to-l from-brand-light dark:from-brand-dark to-transparent"></div>
                        <button
