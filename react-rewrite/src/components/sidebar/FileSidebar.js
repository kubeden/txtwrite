...
const FileItem = ({
    ...
    const getDropIndicatorClasses = () => {
        ...
        switch (position) {
            ...
            case 'inside':
                return item.type === 'folder' ? 'bg-blue-500/10 outline outline-2 outline-blue-500 outline-dashed' : '';
            default:
                return '';
        }
    };
    ...
    const handleItemClick = (e) => {
        if (item.type !== 'folder' && !isRenaming && !confirmDelete && !isMenuOpen) {
            e.stopPropagation();
            if (item.documentRef) {
    ...
    if (item.type === 'folder') {
        return (
            ...
            >
                ...
                >
                    ...
                    >
                        {isExpanded ?
                            <ChevronDown size={14} className="text-neutral-600 dark:text-neutral-400 mr-1" /> :
                            <ChevronRight size={14} className="text-neutral-600 dark:text-neutral-400 mr-1" />
                        }
                        <Folder size={14} className="text-neutral-600 dark:text-neutral-400 mr-2" />
                        {isRenaming ? (
                            <input
                                ref={inputRef}
                                type="text"
                            ...
                            <span className="text-neutral-600 dark:text-neutral-300 truncate">{item.name}</span>
                        )}
                    </div>
                ...
                {isExpanded && item.children && (
                    <div className={isDropTarget && dropTarget.position === 'inside' ? 'pl-2 border-l-2 border-blue-500 border-dashed' : ''}>
        ...
        // File item
        const isActive = item.id === activeDocumentId || item.documentRef === activeDocumentId;
        ...
        return (
            ...
            >
                <div
                    className={`flex items-center py-1 px-2 hover:bg-neutral-200 hover:dark:bg-neutral-800 cursor-pointer text-sm relative ${isActive ? 'bg-neutral-200 dark:bg-neutral-800' : ''
                ...
                >
                    <div className="flex items-center w-full" style={{ paddingLeft: `${paddingLeft}px` }}>
                        {item.type === 'markdown' ? (
                            <FileText size={14} className="text-blue-500 dark:text-blue-400 mr-2" />
                        ) : (
                            <File size={14} className="text-neutral-600 dark:text-neutral-400 mr-2" />
                        )}
                    ...
                    {confirmDelete && (
                        <div
                            ...
                            onClick={(e) => e.stopPropagation()}
                        >
                            <p className="text-neutral-600 dark:text-neutral-300 text-xs mb-2">Delete file "{item.name}"?</p>
                            <div className="flex justify-end gap-2">
                                <button
                                    className="px-2 py-1 bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-300 text-xs rounded hover:bg-neutral-200 dark:hover:bg-neutral-700"
                                    onClick={() => setConfirmDelete(false)}
...
export default function FileSidebar({
    ...
    const handleRename = (itemId, newName) => {
        let updatedFileSystem = updateItemInFileSystem(fileSystem, itemId, item => {
            ...
            if (item.type !== 'folder' && item.documentRef) {
                ...
                const fileName = item.type === 'markdown' && !newName.endsWith('.md') ? `${newName}.md` : newName;
    ...
    const handleDelete = (itemId) => {
        ...
        if (item.parentId) {
            ...
            updatedFileSystem = updateItemInFileSystem(fileSystem, item.parentId, parentItem => ({
                ...parentItem,
    ...
    const handleDrop = (targetItem) => {
        ...
        if (draggedItemData.parentId) {
            ...
            updatedFileSystem = updatedFileSystem.filter(item => item.id !== draggedId);
        }
    ...
    const createDragGhost = (item) => {
        ...
        ghost.className = 'fixed top-0 left-0 bg-neutral-900 border border-blue-500 px-3 py-1 rounded shadow-md opacity-90 z-50 pointer-events-none';
        ...
        if (item.type === 'folder') {
            ...
            icon.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#3B82F6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>';
        }
    ...
    return (
        <div className="mt-4 w-full overflow-y-auto flex flex-col h-full">
            ...
            <div className="flex flex-col items-start justify-start pb-4 pt-2 px-2 gap-y-1 border-t-4 border-neutral-200 dark:border-neutral-800 mt-auto">
                ...
                <a href="https://github.com/kubeden/txtwrite" target="_blank" className="flex items-center py-2 px-2 hover:bg-neutral-200 hover:dark:bg-neutral-800 cursor-pointer text-sm w-full rounded-sm">
                    <span className="text-neutral-600 dark:text-neutral-300">Support</span>
                </a>
                {/* <div className="flex items-center py-1 px-2 hover:bg-neutral-800 cursor-pointer text-sm w-full">
